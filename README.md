...
GraphPad Prism 10 の既定の体裁に合わせている（`build/10_graph.js` の `gopt()` が既定値）：

- 文字はすべて**太字 Arial・黒**（`bold` / `ink`）。軸線 2.6px、目盛は外向き 7px・太さ 2.4px
- 軸線は**目盛の範囲だけ**引き、X軸とY軸が角で接するよう `autoRange()` が範囲を目盛にスナップする
  （目盛は `autoRange` が決め、`startPlot` はそれを再利用する。二重に計算すると間隔が粗くなるため）
- 既定配色は Prism 標準（橙 #F0A22E → 朱 → 暗赤 → 紫 → 青 → 水色）、記号は全データセット円
- データ点・棒・箱・バイオリン・円は**黒縁**、エラーバー・中心線・ブラケットも黒
- 凡例は黒縁の色見本＋太字（XY と生存曲線は線＋記号）
- 有意差ブラケットはクリップ外レイヤに描き、`prepBrackets()` が本数とタイトル高さから上余白を確保する
- ヒートマップは Viridis、相関行列は青-白-赤。どちらも黒枠＋目盛付きカラーバー
